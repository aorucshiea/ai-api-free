...
|----------|-------------|-------|
| **iflow** ⭐ | ~2000 API calls/day, 1 concurrent, includes GLM-4.6, Qwen3, DeepSeek - Meets hobbyist needs |
| **NVIDIA NIM** ⭐ | 1000-5000 credits for development, includes GLM-4.7, MiniMax-M2.1 - Meets hobbyist needs |
...
| **ModelScope (魔塔)** ⭐ | 2000 free calls/day, 500 per model, covers Qwen, DeepSeek, GLM - Meets hobbyist needs |
| **Cherry Studio** | Desktop client with built-in free models, no API key required |
| **anigravity** ⭐ | Weekly free quota for Gemini and Claude latest models - Meets hobbyist needs |
...
| Provider | Description | Notes |
|----------|-------------|-------|
| **AI Tools (platform.aitools.cfd)** ⭐ | Free OpenAI-compatible API, supports DeepSeek-R1/V3, Qwen3, GLM models, no registration required - Meets hobbyist needs |
| **LM Speed (lmspeed.net)** ⭐ | Free AI API provider directory with 33+ free providers, includes unlimited free APIs - Meets hobbyist needs |
| **Free Qwen3** 🆓 | Completely free, no registration required - Meets hobbyist needs |
| **Free QwQ-32B** 🆓 | QwQ-32B distributed API, completely free, no registration, unlimited calls - Meets hobbyist needs |
...
| 供应商 | 描述 | 说明 |
|--------|------|------|
| **iflow** ⭐ | 每天约 2000 次 API 调用，1 并发，包含 GLM-4.6、Qwen3、DeepSeek - 满足业余需求 |
| **NVIDIA NIM** ⭐ | 1000-5000 credits 用于开发，包含 GLM-4.7、MiniMax-M2.1 - 满足业余需求 |
| **SiliconFlow (硅基流动)** | 注册赠送 2000 万 Tokens，9B 以下模型永久免费 |
| **OpenRouter** | 50-1000 次/天 :free 模型调用（取决于余额） |
| **ModelScope (魔塔)** ⭐ | 每天 2000 次免费调用，单模型 500 次/天，覆盖 Qwen、DeepSeek、GLM - 满足业余需求 |
| **Cherry Studio** | 桌面客户端，内置免费模型，无需 API Key |
| **anigravity** ⭐ | 每周可免费使用一定额度的 Gemini 和 Claude 最新模型 - 满足业余需求 |
...
| 供应商 | 描述 | 说明 |
|--------|------|------|
| **Free Qwen3** 🆓 | 完全免费，无需注册 - 满足业余需求 |
| **Free QwQ-32B** 🆓 | QwQ-32B 分布式 API，完全免费，无需注册，无调用上限 - 满足业余需求 |
